...
4. Users requesting the "catalog" directly are now served the rich WhatsApp native catalog component instead of a raw text list, improving conversion rates. The catalog button's body text also dynamically matches the conversation language (English, Gujarati, or Hindi) based on unicode script detection. *(Fixed a bug where it failed to trigger due to checking `botSettings` instead of `tenant` for the catalog ID).*
**Files Changed**:
